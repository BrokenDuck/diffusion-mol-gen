...
See https://arxiv.org/abs/2107.03006
"""
...
class D3PM:
    ...
    def _build_transitions(self, schedule: NoiseSchedule) -> None:
        ...
        for t in range(T):
            ...
            if self.transition == "uniform":
                Qt = (1 - beta_t) * torch.eye(K, dtype=torch.float64) + (
                    beta_t / K
                ) * torch.ones(K, K, dtype=torch.float64)
    ...
    def p_sample(self, pred_logits: Tensor, x_t: Tensor, t: Tensor) -> Tensor:
        ...
        if self.transition == "absorbing":
            ...
            x0_probs = torch.cat(
                [real_probs, torch.zeros_like(real_probs[:, :1])], dim=-1
            )  # [N, K+1]
        else:
